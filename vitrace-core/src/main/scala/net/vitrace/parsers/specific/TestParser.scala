package net.vitrace.parsers.specific
/*
...
class JsonParser1 extends Parser {
  ...
  case class Trace() extends LogLevel
  case class Debug() extends LogLevel
  case class Info() extends LogLevel
  case class Warning() extends LogLevel
  case class Error() extends LogLevel
  case class Fatal() extends LogLevel
...
//  }
  ...
  def parseLogLevel(value: String) = Debug

  def LogLevel = rule { Text ~> (s => parseLogLevel(s))   }

  def parseTime(value: String) = DateTime

  def Time = rule { Text ~> (s => parseTime(s))   }
...
//  }
  ...
  def Mdc = rule { "[ " ~ Text ~~> ArrayNode ~ ":" ~ Text ~ "] " }
