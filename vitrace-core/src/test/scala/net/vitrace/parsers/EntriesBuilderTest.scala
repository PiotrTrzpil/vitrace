...
class EntriesBuilderTest extends FunSuite with RegexParsers
{
   case class LineT0(message : String) extends LogLine { def toMap = Map("message" -> message) }
   case class LineT1(number : Int) extends LogLine { def toMap = Map("number" -> number) }
   case class LineT2(notes : String) extends LogLine { def toMap = Map("notes" -> notes) }

//   test("merge")
//   {
//      val merged = LineT0("mess") merge LineT1(6)
//      val map = merged.toMap
//      assert(map.get("message") === Some("mess"))
//      assert(map.get("number") === Some(6))
//   }
   val testparser = new LogParser {
  val id = "test"
      def parse(line: String, index: Int) = index match {
         case 0 => ParseFailure(line, "")
      }
      val spanSize = 3
   }
//   test("parseFragment")
...
//      val fragment: LogLine = builder.parseFragment(List("", "", ""), 0, testparser)
//      val map = fragment.toMap
//      assert(map.get("message") === Some("mess"))
//      assert(map.get("number") === Some(6))
//      assert(map.get("notes") === Some("n"))
//   }
//   test("parseFragmentTooShort")
//   {
//      val builder = new EntriesBuilder
//      val fragment: LogLine = builder.parseFragment(List("", ""), 0, testparser)
//      val map = fragment.toMap
...
//   {
//      val builder = new EntriesBuilder
//      val fragment = builder.parseLines(testText.split('\n').toList, 0, List(testparser))
//      val logLine = fragment.head match {case x: LogLine => x}
//      val map = logLine.toMap
//      assert(map.get("message") === Some("mess"))
//      assert(map.get("number") === Some(6))
//  //    assert(map.get("notes") === None)
//   }
